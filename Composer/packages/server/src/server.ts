...
export async function start(settings: ApplicationSettings = defaultSettings) {
  // init settings service before anything else
  ...
  pluginLoader.loadPluginsFromFolder(path.resolve(__dirname, '../../../plugins')).then(() => {
    ...
    app.use(`${BASEURL}/api`, authorize, createApiRouter());
